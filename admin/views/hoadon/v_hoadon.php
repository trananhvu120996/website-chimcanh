    ...
    <input class="button" type="submit" value="Tìm " name="btnTim" />
      ...
      <tbody>
        ...
        <tr>
          <td>
          <?php echo $hd->so_hd; ?>
          <a href="chitiethoadon.php?ma_hoa_don=<?php echo $hd->so_hd ?>" title="Xem chi tiết hóa đơn">
	          <img src="public/layout/resources/images/icons/pencil.png" alt="Edit" />
          </a>
          <input type="hidden" value="<?php echo $hd->so_hd ?>" name="ma_hoa_don[]" />
          </td>
          <td><?php echo $hd->Hoten ?></td>
          <td><?php echo date("d/m/Y",strtotime($hd->ngay_hoadon)) ?></td>
          <td><?php echo number_format($hd->trigia_hd) ?></td>
